#!/usr/bin/env node
/**
 * Axon — Gmail OAuth Setup
 * Corre UNA SOLA VEZ en tu PC.
 *
 * Uso: node brain/sync/connectors/gmail/auth.mjs
 */
...
const server = http.createServer(async (req, res) => {
  ...
  console.log(' Gmail conectado. Puedes decirle a Axon "revisa mis correos"');
  console.log('   "Que correos tengo sin leer?"');
  console.log('   "Haz triage de mi inbox"\n');
});

server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error('\n Puerto 3001 ocupado. Cierra la app que lo usa e intenta de nuevo.\n');
  } else {
    console.error('\n Error:', err.message);
  }
  process.exit(1);
...
server.listen(3001, () => {
  console.log('Esperando autorizacion en puerto 3001...\n');
});
